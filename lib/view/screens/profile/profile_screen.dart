        ...
        return authController.profileModel == null ? const Center(child: CircularProgressIndicator()) : ProfileBgWidget(
          ...
          mainWidget: SingleChildScrollView(physics: const BouncingScrollPhysics(), child: Center(child: Container(
            ...
            child: Column(children: [
              ...
              (authController.profileModel != null && authController.profileModel!.earnings == 1) ? Padding(
                padding: const EdgeInsets.only(bottom: Dimensions.paddingSizeSmall),
                child: ProfileButton(icon: Icons.account_balance, title: 'my_account'.tr, onTap: () {
                  Get.toNamed(RouteHelper.getCashInHandRoute());
                }),
              ) : const SizedBox(),

              if(Get.find<SplashController>().configModel!.disbursementType == 'automated' && (authController.profileModel!.type! == 'store_wise' || authController.profileModel!.type! == 'zone_wise') && authController.profileModel!.earnings != 0)
                Column(children: [
                  Padding(
                    padding: const EdgeInsets.only(bottom: Dimensions.paddingSizeSmall),
                    child: ProfileButton(icon: Icons.payments, title: 'disbursement'.tr, onTap: () {
                      Get.toNamed(RouteHelper.getDisbursementRoute());
                    }),
                  ),

                  ProfileButton(icon: Icons.money, title: 'disbursement_methods'.tr, onTap: () {
