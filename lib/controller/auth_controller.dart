...
import 'package:sixam_mart_delivery/data/model/response/wallet_payment_model.dart';
...
class AuthController extends GetxController implements GetxService {
  ...
  Future<void> recordLocation() async {
    ...
    if(Get.find<SplashController>().configModel!.webSocketStatus!) {
      await authRepo.recordWebSocketLocation(_recordLocation!);
    } else {
  ...
  Future<void> getWalletProvidedEarningList() async {
    ...
    Response response = await authRepo.getWalletProvidedEarningList();
    if(response.statusCode == 200) {
      _walletProvidedTransactions = [];
      WalletPaymentModel walletPaymentModel = WalletPaymentModel.fromJson(response.body);
      _walletProvidedTransactions!.addAll(walletPaymentModel.transactions!);
    }else {
      ApiChecker.checkApi(response);
    }
    update();
